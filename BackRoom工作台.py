...
功能：
- 启动程序后在后台静默请求指定的 API 地址（JSON），解析并展示 `data.content.count` 和 `data.content.dyn_list`。
- 支持用户输入刷新间隔（分钟），定时刷新；支持立即刷新。
- 列表可查看每条 dyn 的详情（原始 JSON）并打开跳转链接（若有）。

用法：
    python bili_tribee_viewer.py
